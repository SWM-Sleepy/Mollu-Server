...
class ContentAcceptanceTest extends AcceptanceTest {
    ...
    void 회원가입_이후_컨텐츠_업로드() {
        ...
        // when
        final List<String> groups = 소속_그룹을_조회한다(accessToken);
        final ExtractableResponse<Response> 컨텐츠_업로드_응답 = 컨텐츠_업로드_요청(accessToken, NOW, groups);
    ...
    private List<String> 소속_그룹을_조회한다(String accessToken) {
        final ExtractableResponse<Response> 소속_그룹_응답 = 소속_그룹_조회(accessToken);
        return 소속_그룹_응답.body().jsonPath().getList("groups.groupId");
    }

    @Test
    void 컨텐츠를_업로드_했으면_몰루타임은_null로_조회한다() {
        // given
        final String accessToken = 회원가입_요청_및_응답("google");
        컨텐츠를_업로드_한다(accessToken, NOW.minusDays(1));

        MOLLU_타임_설정();

        final SearchMolluTimeResponse mollu_타임_조회_응답1 = toObject(MOLLU_타임_조회_요청(accessToken), SearchMolluTimeResponse.class);
        assertAll(
                () -> assertThat(mollu_타임_조회_응답1.molluTime()).isNotNull(),
                () -> assertThat(mollu_타임_조회_응답1.question()).isNotNull()
        );
        컨텐츠를_업로드_한다(accessToken, NOW);

        // when
        final SearchMolluTimeResponse mollu_타임_조회_응답2 = toObject(MOLLU_타임_조회_요청(accessToken), SearchMolluTimeResponse.class);

        // then
        assertAll(
                () -> assertThat(mollu_타임_조회_응답2.molluTime()).isNull(),
                () -> assertThat(mollu_타임_조회_응답2.question()).isNull()
        );
    }
    ...
    private void 컨텐츠를_업로드_한다(String accessToken, LocalDateTime uploadDateTime) {
        final List<String> groups = 소속_그룹을_조회한다(accessToken);
        final ExtractableResponse<Response> 컨텐츠_업로드_응답 = 컨텐츠_업로드_요청(accessToken, uploadDateTime, groups);
        assertAll(
                () -> assertThat(컨텐츠_업로드_응답.statusCode()).isEqualTo(HttpStatus.CREATED.value()),
                () -> assertThat(컨텐츠_업로드_응답.header("Location")).isNotNull()
        );
    ...
    @Test
    @DisplayName("신고한 컨텐츠는 조회할 수 없다 v1.1")
    void 신고한_컨텐츠는_조회할_수_없다() {
        // given
        final String other = 다른_사람_회원가입_요청_및_응답("google");
        final String me = 회원가입_요청_및_응답("google");
